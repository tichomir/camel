...
Enforcement modes
-----------------
``EnforcementMode.EVALUATION`` (default):
    On policy denial, raises :class:`PolicyViolationError` immediately with no
    user interaction.  Used for automated benchmarking (NFR-2).

``EnforcementMode.PRODUCTION``:
    On policy denial, invokes the :class:`ConsentCallback` to display the tool
    name, argument summary, and denial reason.  Proceeds on user approval;
    raises :class:`PolicyViolationError` with ``consent_decision="UserRejected"``
    on rejection.  Requires a ``consent_callback`` at construction time.
"""
...
class PolicyViolationError(Exception):
    ...
    consent_decision:
        ``None`` when raised in ``EVALUATION`` mode (automatic denial).
        ``"UserRejected"`` when raised in ``PRODUCTION`` mode after the user
        explicitly rejected the consent prompt.  Callers (e.g. the execution
        loop orchestrator) can use this field to distinguish terminal
        user-rejections from automatic evaluation denials and apply different
        retry logic.
...
class AuditLogEntry:
    ...
    consent_decision:
        ``None`` when outcome is ``"Allowed"`` or when
        ``enforcement_mode`` is ``EVALUATION``.
        ``"UserApproved"`` when a denied call was approved by the user via
        the consent callback (PRODUCTION mode).
        ``"UserRejected"`` when a denied call was explicitly rejected by the
        user via the consent callback (PRODUCTION mode).
...
class EnforcementMode(Enum):
    """Controls how the interpreter handles policy denials.

    Members
    -------
    EVALUATION:
        Raises :class:`PolicyViolationError` immediately on any policy denial,
        with no user interaction.  Used for automated AgentDojo benchmarking
        and unit testing.  (NFR-2: no LLM or UI in the enforcement path.)
    PRODUCTION:
        On a policy denial, invokes the :class:`ConsentCallback` to prompt the
        user before deciding whether to proceed or cancel.  Requires a
        ``consent_callback`` to be supplied at construction time.
...
class ConsentCallback(Protocol):
    """Protocol for user-consent callbacks used in PRODUCTION enforcement mode.

    Any callable matching this signature is accepted — no explicit subclassing
    of this Protocol is required.  This makes it trivial to inject a lambda,
    a mock, or a curried function in tests (NFR-9).

    Parameters
    ----------
    tool_name:
        Name of the tool whose call was denied by policy.
    argument_summary:
        Human-readable summary of the arguments (value types + provenance,
        not raw sensitive data).  Generated by :func:`_summarise_args`.
    denial_reason:
    ...
    -------
    bool
        ``True`` → approve the call and proceed.  ``False`` → reject; a
        :class:`PolicyViolationError` with ``consent_decision="UserRejected"``
        is raised.
    """
...
def _summarise_args(
    ...
    """Build a human-readable argument summary for the consent prompt.

    For each argument the summary includes the raw value representation
    (truncated to 80 characters) and the provenance sources, so the user
    understands *why* a policy denied the call without seeing full sensitive
    data.
    ...
    kwargs_mapping:
        Mapping from argument name to :class:`~camel.value.CaMeLValue`.

    Returns
    -------
    str
        A string of the form
        ``"tool_name(key=repr(raw)[:80] [sources: src1, src2]; ...)"``
    """
    parts: list[str] = []
    for key, cv in kwargs_mapping.items():
        raw_repr = repr(cv.raw)[:80]
        sources_str = ", ".join(sorted(cv.sources)) or "(none)"
        parts.append(f"{key}={raw_repr} [sources: {sources_str}]")
    joined = "; ".join(parts)
...
class CaMeLInterpreter:
    ...
    def _eval_Call(
        ...
        1. **Policy check** (if ``_policy_engine`` is not ``None``):
           ...
           If allowed: append ``AuditLogEntry(outcome="Allowed")`` and proceed.
           If denied (EVALUATION mode): append ``AuditLogEntry(outcome="Denied")``,
           raise :class:`PolicyViolationError`.
           If denied (PRODUCTION mode): invoke ``_consent_callback``; on approval
           append ``AuditLogEntry(consent_decision="UserApproved")`` and proceed;
           on rejection append ``AuditLogEntry(consent_decision="UserRejected")``
           and raise :class:`PolicyViolationError` with
